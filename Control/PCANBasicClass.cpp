...
void PCANBasicClass::LoadAPI()
{
	...
	if(!LoadDllHandle())
	{
		cout << "[Error!]: Unable to find the DLL PCANBasic.dll!" << endl; 
		exit(1);
	}
	else
	{
		cout << "[Info]: DLL is loaded successfully!" << endl;
	}
	...
	if(!m_bWasLoaded)
	{
		cout << "[Error!]: DLL functions could not be loaded!" << endl; 
		exit(1);
	}
	else
	{
		cout << "[Info]: API is loaded successfully!" << endl;
	}
}
...
{   
	...
	//Loads Dll
	//
	m_hDll = LoadLibrary(_T("PCANBasic"));
